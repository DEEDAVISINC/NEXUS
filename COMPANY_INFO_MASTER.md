...
- **Employee/Vendor Numbers (GATEWAY, Jul 2026, finalized after 4 rounds of Dee feedback):** every hire gets a personnel number the moment onboarding starts, format **`[SEQ]-[YYMM]-[EMP|VEN]-[LEVEL]-[DIVISION][ACCOUNT EMOJI]`** — e.g. `0001-2607-EMP-AGT-DPTEⓜ️` = 1st hire that month/type, July 2026, W-2 employee, Agent level, DEPOINTE division, Molina account (Ⓜ️). No `DDI-` prefix (redundant — everything here is DDI, per Dee).
  - **Split into two parts, on purpose:** the **core** (`0001-2607-EMP` — sequence + hire month + worker type) is generated ONCE and **never changes again for that person's entire tenure**, no matter how many times they transfer or get promoted. The **suffix** (`AGT-DPTEⓜ️` — current level/division text + account color emoji) is **mutable** and rebuilds via `PUT /nexus/hr/onboarding/<id>/assignment` any time someone changes accounts, divisions, or gets promoted — every rebuild is logged in the append-only audit trail (old number -> new number), so nothing about assignment history is ever lost even though the visible number changes.
  - **Account is a color emoji, not a text code** — per Dee: *"remove the segement hap molina etc and replace at the end of the number by 🟠, 🟢, 🩵, 🔵, etc, i couldnt find a teal circle, thats why i used the heart."* Current palette (**'NEXUS HR ACCOUNT CODES'** Airtable table, `EMOJI` column) — **not limited to MCO client accounts; also covers DDI's own internal programs**, per Dee: *"for general we will use, ⭕️... shield 🛡️, CTS 🏠, etc"*:

| Account | Emoji | Status |
|---|---|---|
| CareSource (= HAP CareSource, one account — per Dee: *"caresource is hap, its orange"*) | 🟠 | Live |
| Molina Healthcare of Michigan | Ⓜ️ (circled M — per Dee, replaced 🩵) | Live — HCBS PSA Effective 2026-07-21 (NMT + CTS); attestation + Availity approved Jul 29 (Customer ID 3878016) — verify NPI on Availity profile |
...
| **HAVEN** — Housing Assistance Vital Emergency Network, disaster continuity (housing, MOB-B, medical continuity) — sector-vertical program brand | 🚨🏘️ | Proposal/pitch stage — no signed contract yet |
| **VITAL** — Verified Integrated Transport And Logistics, healthcare/medical courier & pharma logistics TPA (VITAL-HS + VITAL-MCO tracks) — sector-vertical program brand | 🚑💊 | Proposal/pitch stage — no signed contract yet |
...
**No emoji appended at all** if the account is blank or has none assigned — never invents a color. Text `ACCOUNT_CODE` values (CSRC, MOLN, HUMN, SHLD, CTS, GEN, etc.) still exist in that table for internal search/filtering — they just no longer appear in the personnel number itself. More accounts/programs ("etc." per Dee) will be added the same way — no code change needed, just `POST /nexus/hr/account-codes`.
  ...
  - **An employee can be tied to MULTIPLE accounts at once** — per Dee, Jul 2026: *"an employee could have multiple segments."* The `account` field holds a **comma-separated list** (e.g. `Molina Healthcare of Michigan, CTS`), and the personnel-number suffix carries **every matched emoji back to back, in order, no separator** — e.g. a supervisor on both Molina and CTS reads `...-SUP-DPTEⓜ️🏠`, and adding SHIELD on top rebuilds it to `...-SUP-DPTEⓜ️🏠🛡️`. Any account with no emoji assigned is silently skipped (never invents one). Tested via `PUT /nexus/hr/onboarding/<id>/assignment` — core stays frozen, suffix rebuilds, full history logged in the audit trail.
...
| Molina Healthcare of Michigan | ✅ **CONTRACTED** — HCBS PSA FFS Effective **2026-07-21** (Vendor ID 214337479 thru 2029-07-31). Attachment B: NMT + CTS. PDF: `BIDS:RESOURCES/MOLINA HIDE SNP LTSS NETWORK/CONTRACTS/Molina_HIDE_SNP_LTSS_PSA_Fully_Executed_2026-07-21.pdf`. **Availity approved Jul 29, 2026** — Customer ID **3878016** / App **63821858**. Attestation done. NEXUS hard gates both `True`. Still verify **NPI 1538939111** on Availity org profile before first claim. Record: `CREDENTIALING/AVAILITY_PORTAL.md` | Troy HQ — same city as DDI |
