...
- **Employee/Vendor Numbers (GATEWAY, Jul 2026, finalized after 2 rounds of Dee feedback):** every hire gets a personnel number the moment onboarding starts, format **`[DIVISION]-[ACCOUNT]-[LEVEL]-[EMP|VEN]-[YYMM]-[SEQ]`** — e.g. `HAVN-CSRC-AGT-EMP-2607-0001` = HAVEN division, CareSource account, Agent level, W-2 employee, started July 2026, 1st in that exact bucket. No `DDI-` prefix (redundant — everything here is DDI, per Dee). `EMP` = W-2 employee, `VEN` = 1099 contractor ("vendor number," Dee's terminology). `ACCOUNT` (which client/MCO contract — **'NEXUS HR ACCOUNT CODES'** Airtable table, e.g. CareSource=CSRC, Molina=MOLN) and `LEVEL` (seniority tier — **'NEXUS HR LEVEL CODES'** Airtable table, e.g. Agent=AGT, Supervisor=SUP, Manager=MGR, Director=DIR) both edit live in Airtable or via `GET/POST /nexus/hr/account-codes` and `/nexus/hr/level-codes` — no code change needed. Blank/unmatched account falls back to `GEN`, blank/unmatched level falls back to `STF` — never fabricated. Answers Dee's question directly: "how do i know that a customer care agent is working in the HAP account, or the MOLINA account... or if they are a manager, supervisor etc" — it's in the ID itself.
