# CWC / DDI Follow-Up & Outreach Calendar

**Created:** May 30, 2026  
**Source:** `SESSION_SUMMARY_MAY_30_2026.md`  
**Cross-ref:** `PENDING_ACTIONS.md` · `CLIENT OUTREACH/LHD_BACKUP_CONTACTS_SIX_COUNTIES.md` · `NEXUS_LEARNING/MICHIGAN_FUNDER_MAP.md`  
**Import all events:** `calendars/CWC_DDI_OUTREACH_FOLLOWUP_JUN_JUL_2026.ics`

---

## Week 1 — June 2–6, 2026

| Date | Time (ET) | Task | Type |
|------|-----------|------|------|
| **Tue Jun 2** | 12:00 PM | Check inbox — Alina Pabin (BCC), Angela Medina/Aimee Surma (MDHHS). **Do not send follow-ups** — check only. If BCC responds → schedule 30-min call + print `ALINA_MEETING_TALK_TRACK.md` | 📧 Monitor |
| **Wed Jun 3** | 1:00 PM | Research MHEF 2026 capacity building deadlines at mihealthfund.org. CWC applicant. Begin concept paper if deadline approaching (Healthcare Access or Family Stability) | 📋 Grant prep |
| **Thu Jun 4** | 1:00 PM | **LinkedIn verify 3 directors:** Oakland, Macomb, Muskegon only (week before outreach). Save names — do not cold-email wrong contacts | 📋 Prep |
| **Fri Jun 5** | 1:00 PM | Research MDHHS HRSN waiver implementation lead (approved Jan 2025). Position DDI/CWC as community delivery partner | 📋 Prep |
| **Sat Jun 6** | 1:00 PM | Research DWIHN subcontracting pathway for Substance Use program (SAMHSA/SOR via PIHP). dwihn.org provider enrollment | 📋 Prep |

---

## Week 2 — June 9–13, 2026

| Date | Time (ET) | Task | Type |
|------|-----------|------|------|
| **Tue Jun 9** | 2:30 PM | **BCC:** If Alina has not responded — one final professional touch. If she schedules → use BCC call prep. **No follow-up after this.** (Strategy review also on calendar 12–2 PM — do BCC check first) | 📧 Outreach |
| **Wed Jun 10** | 1:00 PM | Email Jennifer Casbar (jcasbar@mtm-inc.net) — Access2Care/MTM, Molina sub path. Contract Management NEMT TPA pitch | 📧 Outreach |
| **Fri Jun 12** | 1:00 PM | Research county opioid settlement administrators — Wayne, Oakland, Macomb. Grant cycles + timelines. CWC applicant | 📋 Monitor |

---

## Week 3 — June 16–20, 2026

| Date | Time (ET) | Task | Type |
|------|-----------|------|------|
| **Tue Jun 16** | 12:00 PM | **MDHHS TRIGGER** — If Angela silent → direct LHD. **Wayne (Johnson) + Detroit (Ali Abazeed — separate jurisdiction, engage both)** | 🔥 Decision |
| **Thu Jun 18** | 1:00 PM | LHD direct outreach: Oakland + Macomb (if trigger activated). SHIELD + MDHHS community partner language | 📧 Outreach |
| **Sat Jun 20** | 1:00 PM | LHD direct outreach: Kent (Adam London) + Muskegon. Year 2–3 counties — early relationship building | 📧 Outreach |

---

## July 2026 — Foundation & MCO

| Date | Time (ET) | Task | Type |
|------|-----------|------|------|
| **Tue Jul 7** | 1:00 PM | Research Kresge Foundation Health/Human Services program officer (Troy HQ). Relationship-first — CWC pitch vehicle | 📋 Relationship |
| **Wed Jul 15** | 4:30 PM | MCO community investment outreach (Molina, UHC, Priority) — **no deadline; send when ready.** Package: `MCO_COMMUNITY_INVESTMENT_JUN2026/SEND_TO_MCO/` | 📧 Outreach |

## September–October 2026 — BCBSM Foundation (Winter Cycle)

| Date | Time (ET) | Task | Type |
|------|-----------|------|------|
| **Wed Sep 30** | 1:00 PM | **Begin BCBSM Foundation concept paper** — Healthcare Access primary; Environmental Health secondary if MDHHS momentum supports. Summer cycle skipped. | 📋 Grant prep |
| **Thu Oct 29** | 2:00 PM | **Submit BCBSM Foundation concept paper** — Winter cycle deadline. Form: forms.office.com/r/wZdDebmJf9 | 🔥 Deadline |

---

## Ongoing — First Monday Monthly

| Date | Time (ET) | Task |
|------|-----------|------|
| **Mon Jul 6** | 1:00 PM | Monitor EGrAMS (MDHHS GFOs) + county opioid grants (Wayne/Oakland/Macomb) + CFSEM + MHEF |
| **Mon Aug 3** | 1:00 PM | Same monthly monitor |
| **Mon Sep 7** | 1:00 PM | Same monthly monitor |

---

## Individual `.ics` files

Each event also has a standalone file in `calendars/` prefixed `cwc_outreach_*`.

---

*All task times are 12:00 PM ET or later per scheduling preference.*
