/**
 * Stateless portal auth — HMAC JWT sessions + time-window OTP (no database).
...
function getSecret() {
  const s = process.env.PORTAL_AUTH_SECRET || process.env.NEXUS_EMAIL_PASSWORD || '';
  return s.trim();
}
...
function otpForEmail(email, secret, windowOffset = 0) {
  ...
  const hmac = crypto.createHmac('sha256', secret).update(`portal-otp:${email}:${w}`).digest('hex');
...
function sessionFromEvent(event) {
  ...
  const headerEmail = (event.headers?.['x-portal-email'] || event.headers?.['X-Portal-Email'] || '').trim().toLowerCase();
...
function portalCors(origin) {
  const allowed = [
    'https://portal.deedavis.biz',
    'https://ddi-prism-portal.netlify.app',
  ];
  const req = origin || '';
  let allow = 'https://portal.deedavis.biz';
  if (allowed.includes(req) || req.endsWith('--ddi-prism-portal.netlify.app')) {
    allow = req;
  }
  return {
    'Access-Control-Allow-Origin': allow,
    'Access-Control-Allow-Headers': 'Content-Type, Accept, Authorization, X-Portal-Email',
