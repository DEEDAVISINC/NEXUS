...
const PORTAL_ORIGIN = process.env.PORTAL_PUBLIC_URL || 'https://portal.deedavis.biz';

function buildSignInEmail(email, otp, link) {
  const mins = Math.round(WINDOW_MS / 60000);
  return {
    subject: `Your DDI Service Intake sign-in code: ${otp}`,
    text: `DDI Service Intake — sign in
...
<div style="font-family:Montserrat,Inter,sans-serif;max-width:480px;color:#0B1E3D">
  <p style="font-size:14px;font-weight:700;letter-spacing:.08em;text-transform:uppercase">DDI Service Intake</p>
  ...
  <p style="margin:20px 0">
    <a href="${link}" style="display:inline-block;background:#0B1E3D;color:#C9A84C;padding:14px 24px;text-decoration:none;font-weight:700;font-size:12px;letter-spacing:.12em;text-transform:uppercase;border-radius:6px">Sign in to my dashboard</a>
  </p>
...
exports.handler = async (event) => {
  ...
  try {
    ...
    await transport.sendMail({
      from: `"DDI Service Intake" <${EMAIL_FROM}>`,
      to: email,
    ...
    console.error('portal-auth-send email error:', err);
    return {
      statusCode: 502,
      headers: cors,
      body: JSON.stringify({
