...
| Task | Details | Status |
|---|---|---|
| **Priority Health prism — SET PASSWORD (10-day link)** | Account **approved** Jul 28, 2026 email from `phprism@priorityhealth.com`. Username: **`info@deedavis.biz.prism`**. Click **Set Password** in that email before **~Aug 7, 2026**. Chrome preferred. Support: **800.942.4765 opt 5**. Group/facility access = separate email (~+2 days). **Not DDI PRISM. Portal ≠ contract.** Record: `CREDENTIALING/PRIORITY_HEALTH_PRISM_PORTAL.md` | 🔴 **DO TODAY / THIS WEEK** |
| **VERTEX medical billing — ironclad tranche 1 DONE** | Claim scrub + timely filing + Molina payer ID **38334** + QC no-record block + opt-in API token. Tests: `python3 test_vertex_medical_billing_ironclad.py`. Doc: `VERTEX_MEDICAL_BILLING_IRONCLAD.md`. Still open: 837 submit, ERA/denials, set `VERTEX_NEMT_API_TOKEN` in prod. | ✅ **CODE — Jul 28** |
| **VERTEX — payer profiles WIRED into scrub + claim status** | `vertex_payer_profiles.py` loads `VERTEX_PAYER_PROFILES.json`. Scrub uses profile timely filing; trips stamp clearinghouse IDs; deny stamps Molina dispute **120** / appeal **90**. APIs: `POST .../claims/<id>/submit\|deny\|appeal`, `GET .../payer-profiles`. **Still need:** Dee confirm HAP dispute/appeal days (null today). Next: thin 837/ERA. | ✅ **CODE — Jul 28** |
