...
| **Molina CTS — get PCSP-authorized dollar amount per case** | T2038 is "Manual" pricing on the fee schedule — there is no fixed rate. Before any transition expense is incurred, DDI must get the authorized dollar amount from the LTSS Specialist team (`MHM-LTSS-Specialist@MolinaHealthCare.Com`) and enter it via `PATCH /prism/nemt/cts/<id>` (`pcsp_authorized_amount`). PRISM will not compute a real T2038 claim amount without it. | ⬜ **TODO — per case, when first CTS referral arrives** |
