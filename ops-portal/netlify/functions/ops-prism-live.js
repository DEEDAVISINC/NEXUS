/** Full PRISM live board — supervisors / managers / owners */
...
exports.handler = async (event) => {
  ...
  try {
    const res = await fetch(
      `${OPS_API}/ops/prism/live?email=${encodeURIComponent(refreshed.email)}`,
    ...
    return {
      statusCode: 502,
      headers: cors,
      body: JSON.stringify({ error: 'Could not load live PRISM board', detail: String(err.message || err).slice(0, 120) }),
    };
  }
};
