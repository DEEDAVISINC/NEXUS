/** Supervisor/management oversight — requests | aging | readiness */
...
const PANELS = new Set(['requests', 'aging', 'readiness']);
...
exports.handler = async (event) => {
  ...
  const panel = (qs.panel || 'requests').trim().toLowerCase();
  if (!PANELS.has(panel)) {
    return {
      statusCode: 400,
      headers: cors,
      body: JSON.stringify({ error: 'panel must be requests|aging|readiness' }),
    };
  }

  try {
    const res = await fetch(
      `${OPS_API}/ops/prism/${panel}?email=${encodeURIComponent(refreshed.email)}`,
    ...
    return {
      ...
      headers: cors,
      body: JSON.stringify({
        error: 'Could not load oversight panel',
