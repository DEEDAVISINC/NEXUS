...
exports.handler = async (event) => {
  ...
  try {
    const res = await fetch(
      `${OPS_API}/ops/prism/day?email=${encodeURIComponent(refreshed.email)}`,
    ...
    return {
      statusCode: 502,
      headers: cors,
      body: JSON.stringify({ error: 'Could not load My Day', detail: String(err.message || err).slice(0, 120) }),
    };
  }
};
