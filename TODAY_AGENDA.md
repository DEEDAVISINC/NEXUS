# YOUR AGENDA — Monday, March 23, 2026

**Updated in real-time. This is what's on your plate.**

---

## TOMORROW — TUESDAY MARCH 24, 2026

### LISA TERMORSHUIZEN CALL — Choice Partners / HCDE
**TIME: 2:00 PM CT / 3:00 PM ET — Microsoft Teams**
**Dial-in: +1 346-202-6895,,645136702#**
**Lisa's direct: 713-696-1345 | CPPB, CTCD certified**

**Prep (do BEFORE 3 PM ET):**
- [ ] Choice Partners drug testing cap statement converted to PDF
- [ ] Review pitch document: `CLIENT OUTREACH/CHOICE PARTNERS DRUG TESTING/SEND_TO_BUYER/LISA_CALL_PITCH_MARCH_24.md`
- [ ] Have glance sheet ready (copy to Notes app on phone/iPad)
- [ ] Log into Ionwave (hcdeebid.ionwave.net) and set commodity codes

**This is DDI's door to cooperative purchasing for drug testing nationally. Don't wing it.**

---

## MONDAY MORNING — March 23, 2026 (DO FIRST)

### MCO Lead Screening + NEMT Outreach (4 emails ready)
**File:** `BIDS:RESOURCES/FEDERAL CO OUTREACH PIPELINE/SEND_TO_BUYER/MCO_LEAD_SCREENING_EMAILS_READY.md`
**Attach:** General DDI Capability Statement (PDF)
**DEPENDENCY:** These emails say "DDI is enrolled in MDHHS CHAMPS." Fix NPI login FIRST. If CHAMPS enrollment isn't verified, hold these emails until it is — or remove the CHAMPS claim before sending.

| Order | Action | Target |
|---|---|---|
| **1** | Send EMAIL 1 + start Molina portal registration | MHMProviderContractingMailbox@Molinahealthcare.com |
| **2** | Submit Meridian Contract Request Form + call | mimeridian.com form + 888-773-2647 |
| **3** | Call McLaren + submit via website | 1-888-327-0671 + mclarenhealthplan.org |
| **4** | Send EMAIL 4 to state lead program | MDHHS-CLPPP@michigan.gov |

### NPI FIX — Still Needed for NPPES Updates
- [ ] **Call CMS EUS Help Desk: 1-800-465-3203** (Mon-Fri 7 AM - 7 PM ET)
  - Say: "I need to reset my NPPES account access for NPI 1538939111, Dee Davis Inc. I cannot log in to update my provider information."
  - Have ready: NPI (1538939111), EIN, Authorized Official (Dieasha D. Davis), Address (755 W. Big Beaver Rd., Suite 2020, Troy, MI 48084)
- [ ] **Once back in NPPES, update:**
  - Address: 755 W. Big Beaver Rd., Suite 2020, Troy, MI 48084
  - Taxonomy codes (add lead screening / community health relevant taxonomies)
  - Contact: info@deedavis.biz, 248.376.4550

### CHAMPS NEMT ENROLLMENT — SUBMITTED
**Application #20260323058125 — Submitted March 22, 2026**
- [x] Corrected application submitted as **Atypical Agency / NEMT**
- [ ] **Monitor application status** — check CHAMPS weekly for approval
- [ ] Upon approval: begin MCO credentialing (Molina, UHC, Meridian, HAP, Priority Health, McLaren, Aetna Better Health)

### Other Monday Tasks
- [ ] **CONFIRM CALL with Jean Saporita — National Drug Screening — 321-608-0409** — If no response by noon — call her directly.
- [x] ~~Send USDA REO email to charles.spicer@usda.gov~~ — **REPLIED 03/23: DDI on interested vendors list. Follow up Q3 2026.**

---

## ACTIVE DEADLINES

| Due Date | Opportunity | Agency | Solicitation | Action Needed |
|---|---|---|---|---|
| **March 24 (Tue)** | Call with Lisa TerMorshuizen | Choice Partners / HCDE | Cooperative contract | **CONFIRMED: Tue 2:00 PM CT / 3:00 PM ET.** Teams. |
| **March 26 (Thu)** | PSTA Pre-Proposal Conference | PSTA (St. Petersburg FL) | 26-280182 Background Checks | **11:00 AM EST Teams** — attend if NCS partnership active. BLOCKED on NCS. |
| **March 31** | Document Shredding | VA Benefits Administration | 36C10D26Q0034 | Pull solicitation from SAM.gov. Review scope. Find shredding sub. |
| **April 10** | PSTA Background Check RFP | PSTA | 26-280182 | DUE 1:00 PM EST — BLOCKED on NCS partnership |

---

## FOLLOW-UPS DUE THIS WEEK

| Date | Contact | Agency | Action |
|---|---|---|---|
| **Mar 24** | James Postell | VA NCO 21 | Follow up — NorCal Sterile Processing Courier |
| **Mar 24** | Kerri Bielski | Michigan Veteran Homes | Follow up — Lab Testing Services |
| **Mar 25** | 15+ VA COs | Various VA NCOs | Follow up — all courier/lab/pharmacy emails sent Mar 18 |
| **Mar 25** | Brittney Miller | SEC | Follow up — Process Server Services |
| **Mar 25** | NIAID | NIH | Follow up — Medical Courier Services |
| **Mar 25** | Joe Wells / James Johnson | DHA JBSA | Follow up — Medical Courier BPA |
| **Mar 26** | Tracy Riley | ICE DHS | Follow up — Courier Services |
| **Mar 26** | David Walther | ICE DHS | Follow up — Fingerprinting/DNA/E-Verify |
| **Mar 26** | OMNIA Partners | Cooperative | Follow up if no Membership ID received yet |
| **Mar 26** | NCS (National Crime Search) | CRA Partner | Follow up if no response to partner signup |

---

## WAITING FOR REPLIES

| Contact | Agency | What We Sent | What We're Waiting For |
|---|---|---|---|
| Tracy Riley + Mark + Marilyn | ICE — Program Office | **REPLIED 03/23.** DDI sent full capabilities (courier, notary, apostille, DNA, fingerprinting) | Program office response / next steps |
| Matheson Gas (liquid div rep) | Supplier — LN2 | Robbie routing to liquid division | Contact from their Rockville-area rep |
| Nien-Tzu Grace Wang | HHS OMAS | LN2 Sole Source Challenge | Outcome — will they open to competition? |
| HUD FSM | HUD — REO Field Inspection | EDWOSB intro + cap statement | Response |
| VA REO (3 contacts) | VA — REO Field Inspection | EDWOSB intro + cap statement | Response |
| OMNIA Partners | Cooperative Purchasing | Follow-up email sent 03/22 | Membership ID (submitted 02/16 — overdue) |
| NCS (National Crime Search) | CRA Partner | Partner signup submitted 03/22 | Response / account activation |
| Jean Saporita | National Drug Screening | Proposed Monday call | Time confirmation |

---

## COMPLETED THIS WEEK

- [x] All capability statements updated — Landstar removed, Trucking Planet added
- [x] HUD FSM REO Field Inspection cap statement — visibility fixed, differentiators added
- [x] REO Field Inspection outreach emails — HUD FSM sent, VA REO (3) sent, USDA drafted
- [x] Michigan MCO Lead Screening map — 9 MCOs identified, top 4 prioritized
- [x] MCO outreach emails drafted — Molina, Meridian, McLaren, MDHHS CLPPP
- [x] NEMT added to all MCO outreach emails as second service lane
- [x] Choice Partners Drug Testing cap statement — BUILT (Medical Blue + Red)
- [x] ICE Identity Verification cap statement — BUILT (Navy + Gold) — sent to David Walther
- [x] ICE Courier/Logistics cap statement — BUILT (Purple + Teal) — sent to Tracy Riley
- [x] Lisa call pitch document — COMPLETE (435 lines, competitive intel, three-tier model)
- [x] CHAMPS NEMT enrollment submitted — Application #20260323058125
- [x] OLIMP warehouse network documented
- [x] Charles Spicer (USDA RD) replied — DDI on interested vendors list. Follow up Q3 2026.
- [x] Cooperative purchasing landscape mapped — Sourcewell, OMNIA, TIPS, BuyBoard
- [x] OMNIA Partners follow-up sent (membership submitted 02/16, 5+ weeks overdue)
- [x] NCS (National Crime Search) CRA partner signup submitted
- [x] 8(a) and DBE certification requirements outlined
- [x] TIPS USA and BuyBoard status corrected — DDI is registered with both
- [x] **Medical Tourism Transportation — new DDCSS service lane documented** (service catalog + DDCSS execution plan)
- [x] **DDI Certified Medical Transport Specialist program designed** (post-surgical patient handoff, certification DDI owns)
- [x] **Uber Black fulfillment model + Uber corporate partnership strategy documented** (the billion-dollar play)

---

## COOPERATIVE PURCHASING EXPANSION

| Cooperative | DDI Status | Drug Testing Contract | Action Needed |
|---|---|---|---|
| **Choice Partners** | ✅ Registered | Rebid June 2027 | Lisa call March 24 |
| **Sourcewell** | ✅ Registered | Redwood Toxicology expires Feb 2027 | Watch for resolicit |
| **OMNIA Partners** | ✅ Done — no supplier registration exists. Mailing list submitted. | Accurate/First Advantage expire Dec 2027 | Watch for RFP when contract rebids. No more action needed. |
| **TIPS USA** | ✅ Registered | Unknown — check for drug testing category | Search for drug testing contracts/RFPs on TIPS |
| **BuyBoard** | ✅ Registered | Unknown — check for drug testing category | Search for drug testing contracts on BuyBoard |
| **MMCAP Infuse** | ⬜ Not researched | Has DOT/non-DOT contracts | Research current contracts and rebid schedule |

---

## PREP NEEDED BEFORE MARCH 24

For the Lisa TerMorshuizen / Choice Partners call:
- [x] Call pitch document — DONE
- [x] Study script + glance sheet — DONE
- [ ] Choice Partners drug testing cap statement converted to PDF
- [ ] Log into Ionwave (hcdeebid.ionwave.net) and set commodity codes
- [ ] Calendar reminder: Choice Partners Drug & Alcohol Testing rebid ~Q2 2027

---

## PREP NEEDED BEFORE MARCH 31

For the VA Document Shredding bid (36C10D26Q0034):
- [ ] Pull full solicitation from SAM.gov
- [ ] Review scope, CLINs, evaluation method
- [ ] Find local shredding subcontractor
- [ ] Revenue analysis
- [ ] Go/no-go decision
- [ ] If GO — build bid folder, generate response

---

## SATURDAY MARCH 28 — NEXUS AI COPILOT BUILD

**Goal:** Upgrade NEXUS AI Copilot so Dee can chat from nexus.deedavis.biz on any device.
**Build plan (4-5 hours):** Claude API integration, Airtable context injection, quick actions, deploy.

---

## PORTAL REGISTRATIONS PENDING

- [ ] Register on Molina provider portal
- [ ] Register on Meridian provider portal
- [ ] Register on McLaren provider portal
- [x] ~~OMNIA Partners~~ — Membership submitted. Follow-up sent 03/22. Awaiting Membership ID.
- [ ] **Call NCS partner program** — background screening CRA partnership (blocks PSTA bid)

## 8(a) & DBE CERTIFICATION — SCHEDULE A WORK SESSION

**Do NOT rush these. Dedicate a full session to each one.**
**When ready: tell me "let's work on 8(a)" and I'll walk through it.**

## PSTA BACKGROUND CHECK RFP — 26-280182 (BLOCKED ON NCS PARTNERSHIP)

| Field | Detail |
|---|---|
| **Agency** | Pinellas Suncoast Transit Authority, St. Petersburg FL |
| **CO** | Edith Randle — erandle@psta.net |
| **Pre-Proposal Conference** | **Thu March 26, 11:00 AM EST** (Teams) |
| **Due** | **Fri April 10, 2026, 1:00 PM EST** |
| **Blocker** | Need CRA partner (NCS) before DDI can bid |

---

*Updated: Sunday, March 22, 2026 — Goodnight*
