# TODAY'S AGENDA — Sunday, May 11, 2026

---

## TOP PRIORITIES

### 1. Happy Mother's Day
- Enjoy the day. You earned it.

### 2. HAVEN PARTNER OUTREACH — SEND FIRST ✅ READY
**Partners BEFORE MCOs** — so when an MCO asks "who's in your network?", you have an answer.

**23 partner emails ready:**
| Category | Emails | File |
|----------|--------|------|
| **Transport** (TX, FL, LA, SC) | 10 | `HAVEN_PARTNER_OUTREACH_TRANSPORT.md` |
| **Housing — Hotels** | 5 | `HAVEN_PARTNER_OUTREACH_HOUSING.md` |
| **Housing — Modular/Prefab** | 3 | `HAVEN_PARTNER_OUTREACH_HOUSING.md` |
| **Medical** (Home health) | 5 | `HAVEN_PARTNER_OUTREACH_MEDICAL.md` |

⭐ **PRIORITY: SKYLINE CHAMPION**
- Joseph Kimmell, EVP US Operations — 574-971-0018
- **$800M FEMA contract** for manufactured housing units
- Troy, MI based

**Before sending:** Convert one-pagers to PDF:
- `NEXUS_LEARNING/HAVEN_Transport_Partnership_OnePager.html`
- `NEXUS_LEARNING/HAVEN_Housing_Partnership_OnePager.html`
- `NEXUS_LEARNING/HAVEN_Medical_Partnership_OnePager.html`

**Priority send order:**
1. **Skyline Champion** — Outreach (Joseph Kimmell — professional pitch first)
2. **Transport** — Wheelchair/stretcher fleets in TX, FL, LA, SC
3. **Housing** — Red Roof, Extended Stay, WoodSpring, La Quinta, Motel 6, Clayton, Boxabl
4. **Medical** — Amedisys, Enhabit, VitalCaring, LHC, CenterWell

### 3. THEN: Send MCO Outreach Emails (NEMT + HAVEN)
- **38 emails ready across 9 states** — all in `CLIENT OUTREACH/[STATE] MCO NEMT HAVEN/` folders
- **Attachment for every MCO email:** Convert `NEXUS_LEARNING/HAVEN_Master_Proposal.html` → PDF (`HAVEN_Master_Proposal_DDI_MCO.pdf`). **Do not** attach partner one-pagers to MCOs.
- **Priority send order:**
  1. **Texas** — Community Health Choice follow-up (overdue 23 days) + 6 new MCOs
  2. **South Carolina** — Brand new market, no incumbents for dual-eligible NEMT
  3. **Florida** — 7 MCOs, hurricane season starts June 1
  4. **Georgia** — CareSource (existing partner) is the warmest pitch
  5. **Mississippi** — TrueCare/CareSource relationship play
  6. **Louisiana** — LaGov registered, UHC exit growth
  7. **Ohio** — Anthem follow-up overdue + 4 new MCOs
  8. **Tennessee** — 3 MCOs, BCBS connection
  9. **Alabama** — 1 exploratory email (state-level)

### 4. Region 10 MCO Follow-Ups (7-day)
- May 11 = 7-day follow-up for Michigan MCOs that haven't responded
- Check: Aetna, AmeriHealth, Humana, UHC, Priority Health, Meridian
- Molina: Portal enrollment in progress — separate track

---

## PENDING RESPONSES

| Contact | Last Outreach | Notes |
|---|---|---|
| Alina Pabin (BCBSM) | May 6 | Scheduling 30-min call — waiting on her availability |
| DDC Laboratories | May 8 | Government pricing inquiry — follow up if no response by May 12 |
| MDHHS (Angela/Aimee) | May 3 | SHIELD package sent — awaiting meeting scheduling |
| Lyft Healthcare | May 8 | Contact form submitted — need response for transport network |
| Uber Health | Pending | Need to reach out — healthcare enterprise partnerships |
| Community Health Choice TX | Apr 16 | Forwarded to TPA team — follow-up OVERDUE |

---

## UPCOMING DEADLINES

| Date | Opportunity | Action |
|---|---|---|
| May 11 | Maryland MVA Courier | ITQ submission — warehouse partner search active |
| May 11 | Region 10 MCO Follow-ups | 7-day follow-up for non-responders |
| ~May 15 | DHA Fingerprinting | RFQ expected to post — cap statement ready |
| May 18 | University Health TX Courier | Two RFPs (Pharmacy + Lab) |
| June 1 | Hurricane season starts | FL + LA + MS + SC + TX HAVEN pitch urgency increases |
| June 1 | BCBSM Alina | Follow-up if no meeting by Memorial Day |

---

## WHAT WAS ACCOMPLISHED MAY 9

- Built HAVEN system in NEXUS (7 operational sections)
- Integrated disaster watch (FEMA + NWS live data)
- Created automated partner outreach engine
- Built one-pagers: Transport, Housing, Medical partnerships
- Researched and drafted **38 MCO outreach emails** across 10 states
- Identified named contacts with direct emails for majority of MCOs
- Discovered SC dual-eligible managed care expansion (Jan 2026) — major opportunity
- Discovered UHC exited Louisiana Medicaid (Apr 2026) — growth moment for remaining MCOs
- Registered DDI in LaGov (Vendor ID V31033172101)
- Corrected CHAMPS Provider ID across all files (6309049)
- Updated NEMT expansion strategy with MCO-by-state map
- Created Ohio, Texas, Florida, Louisiana, Georgia, Alabama, Mississippi, Tennessee, South Carolina outreach packages

## WHAT WAS ACCOMPLISHED MAY 10 (This Morning)

**Partner network research completed — 20 outreach emails ready:**
- Identified **24 regional transport companies** across TX, FL, LA, SC (wheelchair/stretcher fleets)
- Identified **5 national hotel chains** with emergency/healthcare housing programs
- Identified **7 home health agencies** with hurricane corridor coverage
- Created **HAVEN_PARTNER_NETWORK_REGISTRY.md** — complete database of all partners
- Generated **10 transport partner emails** (3 TX, 3 FL, 2 LA, 2 SC)
- Generated **5 housing partner emails** (Red Roof, ESA, WoodSpring, La Quinta, Motel 6)
- Generated **5 medical partner emails** (Amedisys, Enhabit, VitalCaring, LHC, CenterWell)

**Sequencing updated:** Partners FIRST → then MCOs

---

*Happy Mother's Day, Dee. Partner outreach ready. MCO outreach ready. The pipeline is loaded.*
