# DEE DAVIS INC — PROFESSIONAL SERVICES PRICING
## Fingerprinting | Drug Testing | Notary | Apostille | Legal Courier | Document Services

**Effective:** February 2026 *(§3 April 2026 — $10/act; witness $50 standalone / $25 with stamps; +$75 mobile)*  
**Service Area:** Michigan Statewide (Mobile)
**Appointments Required | Walk-ins Call Ahead**

---

## SERVICE LINES — NOT THE SAME PRODUCT

**Standalone notary public work, loan signing, apostille, estate/CNTDA delivery, permit running, and courier** are **separate service lines** with **separate pricing**. A Michigan notary commission may be **used inside** more than one line of business, but **clients are billed for the service they bought** — not under a single “notary” label for everything.

| Service line | Section | What the client is buying |
|---|---|---|
| **Notary Public** (MiLONA) | §3 | Acknowledgments, jurats, oaths, general in-person/RON **notarization** — priced per **notarial act** (+ travel when applicable). |
| **Loan signing / Signing agent** | §7 | **Real estate closing** packages, lender workflow, table funding prep — **signing-agent** pricing (not the same as §3 walk-in notary). |
| **Estate / CNTDA** | §4 | Trust delivery, estate document execution, POA/AHD packages — **certified document agent** work; notarizations may be **included in the package**. |
| **Apostille & authentication** | §5 | SOS/State/embassy **authentication** chain — separate from §3 unless the client **only** orders notarization. |
| **Permit runner (NPR)** | §6A | Building permit pull, recording, **permit-line** workflow — **not** §3 (notary may be required **on a form**; still billed as permit-runner service). |
| **Legal courier / runner** | §6 | Filings, retrievals, SOS — distinct from §3 and §6A. |

**Quote rule:** Proposals and invoices name the **primary service line**; statutory **$10/notarial act** appears when §3-style acts are itemized.

---

## 1. FINGERPRINTING SERVICES

| Service | Price | Notes |
|---------|-------|-------|
| Standard Fingerprinting (1-15 cards) | $50/card + travel | Prints stored on file 1 year |
| Volume (16-50 cards/month) | $45/card | Travel included within Michigan |
| Enterprise (51+ cards/month) | $40/card | Travel included, scheduled routes |
| Additional Cards (prints on file) | $25-35/card | No reprinting — reorder anytime within 1 year |
| Livescan (electronic submission) | $75 | Where accepted |
| Ink Card (FD-258 standard) | Included | FBI/state standard cards |

*Mobile/travel fees apply to all on-site services — see Travel Fees section below*

---

## 2. DRUG TESTING SERVICES

### Non-DOT Testing

| Test | Price | Results |
|------|-------|---------|
| 5-Panel Urine (rapid/instant) | $55 | 15 minutes |
| 10-Panel Urine (rapid/instant) | $70 | 15 minutes |
| 5-Panel Urine (lab-confirmed) | $85 | 1-3 days |
| 10-Panel Urine (lab-confirmed) | $125 | 1-3 days |
| 5-Panel Hair Follicle | $175 | 90-day detection window |
| 10-Panel Hair Follicle | $225 | 90-day detection window |
| Breath Alcohol Test (BAT) | $75 | Instant |
| ETG Alcohol Urine | $65 | 80-hour detection |

### DOT Testing (Federally Regulated)

| Test | Price | Notes |
|------|-------|-------|
| DOT 5-Panel Urine (SAMHSA lab) | $125 | CDL, pipeline, transit, aviation |
| DOT Breath Alcohol (BAT) | $85 | Certified BAT |
| DOT Random Testing Program | $250/year | Selection, scheduling, compliance |
| DOT Post-Accident | $150 | Within 32 hours |
| DOT Return-to-Duty | $150 | After SAP evaluation |

### Occupational Health Screening

| Service | Price | Notes |
|---------|-------|-------|
| Pre-Employment Physical | Call for pricing | Brokered through provider |
| TB Test | Call for pricing | Brokered through provider |
| Respirator Fit Test | Call for pricing | Brokered through provider |

---

## 3. NOTARY PUBLIC SERVICES

### Michigan fee rules (MiLONA) — on file for compliance

**Law:** Michigan Law on Notarial Acts (MiLONA), Act 238 of 2003 — **MCL 55.285(7)**.  
**Official text (subsection (7)):**

> *(7) The fee charged by a notary public for performing a notarial act shall not be more than $10.00 for any individual transaction or notarial act. A notary public shall either conspicuously display a sign or expressly advise an individual concerning the fee amount to be charged for a notarial act before the notary public performs the act. Before the notary public commences to travel in order to perform a notarial act, the notary public and client may agree concerning a separate travel fee to be charged by the notary public for traveling to perform the notarial act.*

**Source:** [Michigan Legislature — MCL 55.285](https://www.legislature.mi.gov/Laws/MCL?objectName=mcl-55-285) (Michigan Compiled Laws; verify current text if statute is amended).

**Disclaimer:** This document is **Dee Davis Inc.’s internal pricing and compliance reference**, not legal advice. Statutory language is quoted or summarized for **operational clarity**; it does not create an attorney–client relationship. For **interpretation of law** or **specific transactions**, consult **qualified legal counsel** or the **Michigan Secretary of State** notary resources. DDI does not warrant that this page reflects every amendment to MiLONA or related rules.

**How this page applies it**

- **Statutory cap:** The fee for a notarial act **shall not be more than $10.00** for any **individual transaction or notarial act**. That is the **maximum**; a notary may charge **less**. Each acknowledgment, jurat, oath/affirmation, or other **completed notarial act** is its own fee event (not a vague “per signature” unless the act itself is defined that way). Charging more than the statutory maximum for the **notarial fee** can constitute **official misconduct** under MiLONA (see `COMPLIANCE_KNOWLEDGE/NOTARY_REFERENCE.md`).
- **Disclosure:** Fees must be **conspicuously displayed** or **expressly advised before** the act — DDI quotes/invoices satisfy **advance disclosure** of notarial-act fees vs. travel and other components.
- **Travel / mobile / scheduling / after-hours / facility surcharges** are **separate** from the notary fee. Michigan allows a **separate travel fee** **agreed before travel begins**; **all fees** must be **clear to the signer before the notarial act** (see [Michigan SOS — Notary Services](https://www.michigan.gov/sos/notary-services)).
- **DDI practice:** Quotes and invoices show **total** pricing where appropriate; clients may request a **breakout** of notarial-act fees vs. witnessing vs. mobile/convenience fees. **Witnessing** is priced **by context**: **$50.00** when witnessing is the **only** service on the appointment; **$25.00** when witnessing is **on the same appointment as** one or more notarial acts (“accompanied with stamps”). **All-in minimum** for a typical job with stamps = (number of notarial acts × $10) + ($25 if witness + stamps, or $50 if witness-only) + ($75 mobile if applicable) + surcharges below. **Do not** mix §3 notary line items into **loan signing (§7), CNTDA (§4), or permit runner (§6A)** without showing the **primary service** — those lines have their **own** tables below.

| Component | Price | Notes |
|-----------|-------|-------|
| **Notarial act** (each stamp / completed act) | **$10.00** | **Michigan statutory maximum** per act (MCL 55.285(7)); multiply by count of acts |
| **Witnessing — with notarial act(s) same appointment** | **$25.00** | When the job includes **one or more** §3 notarial acts **and** witnessing |
| **Witnessing — only service** (no notarial acts on visit) | **$50.00** | Standalone witness engagement (no stamps / notarial fees on that appointment) |
| **Mobile** (your location) | **$75.00** | Flat mobile / travel-service add-on for the appointment (in addition to per-act fees) |
| Remote Online Notarization (RON) | $25/session or per platform quote | Via ZigSig; **session/agency** pricing (not the same as in-person §3 stack) |
| After-Hours/Weekend Notary | +$50 surcharge | Evenings/weekends (on top of components above) |
| Hospital/Jail/Facility Notary | +$75 surcharge | Specialized locations (on top of components above) |

**Example — minimum for one mobile job (witness + stamps):** **10 notarial acts** + **one witness (bundled)** + **mobile**  
→ (10 × $10.00) + $25.00 + $75.00 = **$200.00 minimum** (before after-hours/facility surcharges).

**Example — witness-only:** **Witnessing only** + **mobile** → $50.00 + $75.00 = **$125.00 minimum** (no notarial acts on that appointment).

**Source of truth (regulatory):** `COMPLIANCE_KNOWLEDGE/NOTARY_REFERENCE.md` — update if the Legislature or SOS changes the notarial-fee cap.

---

## 4. CNTDA — CERTIFIED NOTARY TRUST DELIVERY AGENT

*Estate / trust **document delivery and execution** — separate from **§3 general notary** and **§7 loan signing**. Notarial acts may occur inside these packages; pricing is **package- and role-based**, not “§3 only.”*

| Service | Price | Notes |
|---------|-------|-------|
| Living Trust Package Signing | $150-250 | 6-8+ notarizations per package |
| Estate Planning Document Delivery | $150-250 | Presented & executed by certified agent |
| Power of Attorney Signing | $75-125 | Per document set |
| Advanced Healthcare Directive | $75-125 | Per document set |

*CNTDA-credentialed agents are trained to present and execute trust documents while maintaining compliance with unauthorized practice of law rules.*

---

## 5. APOSTILLE & AUTHENTICATION SERVICES

*Authentication / legalization **after** notarization when required — **separate service** from §3. Client may need §3 + §5 in sequence on the same document; **each section bills separately**.*

| Service | Price | Turnaround |
|---------|-------|-----------|
| State Apostille (Michigan SOS) | $150 | 5-7 business days |
| State Apostille (Rush) | $250 | 1-3 business days |
| Federal Apostille (U.S. Dept of State) | $250 | 2-4 weeks |
| Federal Apostille (Expedited) | $400 | 5-10 business days |
| Document Authentication | $150 | Per document |
| Additional Documents (same order) | $50/each | After first document |
| Embassy/Consulate Legalization | Call for pricing | Country-specific |

*Includes pickup, processing, and return delivery*

---

## 6. LEGAL COURIER & DOCUMENT RUNNER

| Service | Price | Notes |
|---------|-------|-------|
| Court Filing (local courthouse) | $50 + filing fees | Same-day filing |
| Court Filing (rush/same-day) | $75 + filing fees | Priority handling |
| Document Pickup/Delivery (local) | $35 | Within Zone 1-2 |
| Document Pickup/Delivery (regional) | $65 | Zone 3 |
| Document Retrieval (courthouse records) | $50 + copy fees | Certified copies, case files |
| Process Serving | $75/attempt | 15 min on-site wait included |
| Process Serving (additional attempts) | $40/attempt | After first attempt |
| Secretary of State Filings | $50 + state fees | Articles, amendments, good standing |
| Secretary of State (Rush/Expedited) | $100 + state fees | Same-day processing |

---

## 6A. PERMIT RUNNER (NPR) — BUILDING & LARA PERMITS

*Separate from **§3 notary** and **§6 general courier**. Pulling permits, plan review drops, and recording often **require notarized forms** — that does **not** reclassify the job as “§3 only”; bill **permit-runner** service + applicable **§3** notarial acts when itemized.*

| Service | Price | Notes |
|---------|-------|-------|
| Permit Running (LARA / local building) | $75 + permit fees | Business permits, contractor permits — see also service catalog NPR tier |
| Multiple permits (volume) | Custom | Per catalog tiers for 2+ pulls |

---

## 6B. MEDICAL & PHARMACEUTICAL COURIER

| Service | Price | Notes |
|---------|-------|-------|
| Standard Medical Specimen Transport | $50 | Lab samples, bloodwork, pathology |
| STAT/Expedited Specimen Transport | $85 | Priority, time-sensitive samples |
| Pharmaceutical Delivery (local) | $50 | Prescriptions, pharmacy-to-patient |
| Pharmaceutical Delivery (regional) | $75 | Zone 2-3 |
| Pharmaceutical Delivery (STAT/rush) | $125 | Under 2-hour delivery window |
| Medical Records Transport | $50 | HIPAA-compliant, chain of custody |
| Medical Equipment/Supply Delivery | $50-100 | Based on size and distance |
| Organ/Tissue Transport Coordination | Call for pricing | Time-critical, brokered |
| After-Hours Medical Courier | +$75 surcharge | Evenings, weekends |
| Scheduled Daily Route (contract) | Custom pricing | Recurring pickup/delivery runs |
| Temperature-Controlled Transport | +$25 surcharge | Cold chain, insulated packaging |

### Medical Courier Notes:
- All medical couriers are HIPAA-compliant
- Chain of custody documentation maintained
- DDI holds MC# 1647572 (licensed freight broker) and US DOT# 4250594
- Temperature-sensitive items handled with appropriate packaging
- Scheduled daily/weekly routes available for labs, clinics, pharmacies
- **Mobile travel fees apply on top of service fees**

---

## 7. SIGNING AGENT SERVICES (LOAN & REAL ESTATE — NOT §3 NOTARY)

**Loan signing is not the same product as §3 Notary Public.** Signing agents manage **lender/title packages**, timing, and often multiple notarizations in one **closing** — priced as **signing services** below. Itemize **§3**-style notarial fees **only** when separately quoted for compliance or client request.

| Service | Price | Notes |
|---------|-------|-------|
| Loan Signing (standard) | $125-175 | Mortgage, refinance closings |
| Loan Signing (reverse mortgage) | $175-250 | Complex documents |
| Loan Signing (seller package) | $75-100 | Seller-side documents |
| General Document Signing | $75-125 | Non-loan packages |
| E-Signing (hybrid/RON) | $75-100 | Remote portion |

*Available through Snapdocs and direct scheduling*

---

## 8. BACKGROUND CHECK SERVICES

| Service | Price | Notes |
|---------|-------|-------|
| State Criminal Background Check | $30-50 | Michigan ICHAT |
| FBI Background Check (fingerprint-based) | $50 + FBI fee | Via livescan or ink card |
| Employment Background Check | $40-75 | Criminal + employment verification |
| Comprehensive Background Package | $100-150 | Criminal + employment + education + references |

---

## MOBILE TRAVEL FEES (Applies to ALL On-Site Services)

**Any service performed at the client's location includes a mobile travel fee.**
This applies to: fingerprinting, drug testing, notary, signings, courier, trust delivery — everything.

| Zone | Distance from Troy | Fee |
|------|-------------------|-----|
| Zone 1 | 0-15 miles (Troy, Birmingham, Royal Oak, Southfield, Pontiac) | $25 |
| Zone 2 | 15-30 miles (Detroit, Warren, Sterling Heights, Ann Arbor) | $50 |
| Zone 3 | 30-50 miles (Flint, Lansing area, Monroe) | $75 |
| Zone 4 | 50+ miles (Grand Rapids, Saginaw, Kalamazoo) | $100+ |

*Volume/Enterprise fingerprinting accounts: travel included in per-card rate*

---

## SURCHARGES (On Top of Service + Travel)

| Type | Fee |
|------|-----|
| After-Hours (6 PM - 8 AM) | +$75 |
| Weekend (Saturday/Sunday) | +$50 |
| Holiday | +$100 |
| Express/Rush Results | +$30 |
| Emergency (under 2-hour notice) | +$100 |

---

## PAYMENT

- Cash, Check, Credit/Debit Card, Zelle, CashApp
- Net 15/Net 30 available for account clients
- Volume accounts invoiced monthly

---

## CONTACT

**Dee Davis Inc.**
755 W. Big Beaver Rd., Suite 2020
Troy, Michigan 48084
Phone: 248.376.4550
Email: info@deedavis.biz
Web: deedavis.biz

EDWOSB/WOSB Certified | NPI: 153893911

---

*Pricing subject to change. Travel fees apply outside Zone 1.
Government and volume discounts available.*

*Created: February 12, 2026 — Notary §3 MiLONA compliance language added April 2026.*
