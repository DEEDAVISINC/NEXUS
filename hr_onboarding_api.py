...
and automated"). Table starts EMPTY (no role fabricated without Dee's
sign-off) — until rows exist, role_requires_company_email() returns False
for everyone, which is the safe default (no unrequested aliases). HR can
also override the role default either direction per hire via
`provisionCompanyEmail: true/false` on the POST /nexus/hr/onboarding body
(captured as companyEmailOverride, acted on later — see below).
