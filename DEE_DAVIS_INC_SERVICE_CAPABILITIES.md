# DEE DAVIS INC - ACTUAL SERVICE CAPABILITIES
## For EDWOSB Opportunity Mining & Forecasting

**Last Updated:** March 14, 2026  
**Purpose:** Reference for identifying relevant EDWOSB solicitations

---

## 🎯 DDI IDENTITY

**Dee Davis Inc. is a federally certified EDWOSB contract management firm and business operating system focused on all forms of business continuity.** Every capability below serves this mission — ensuring client operations never stop, powered by proprietary AI technology platforms (NEXUS, GPSS, ATLAS PM, FleetFlow™, COMPASS™, PRISM, DDCSS, VERTEX, GBIS) built in-house by founder Dee Davis.

---

## ✅ CORE SERVICE CAPABILITIES

### **1. PROJECT EXECUTIVE SERVICES**

- Enterprise project management
- Government contract execution
- Multi-stakeholder coordination
- Crisis coordination
- PMBOK framework implementation
- Complex project orchestration
- Contract management
- Strategic project planning
- Regulatory compliance management
- Performance monitoring & reporting

**Technology Platform:** ATLAS PM (AI-enabled enterprise intelligence)

**Solicitation Keywords:**
- Project management services
- Program management
- Project executive services
- Contract management
- Enterprise project coordination
- Government contract execution
- PMO services
- Project controls
- Strategic planning services
- Performance management

---

### **2. LOGISTICS & FLEET OPTIMIZATION**

- Freight brokerage consulting (MC-1647572)
- Dispatch solutions
- Route optimization
- Carrier coordination
- Supply chain consulting
- Fleet management consulting
- Transportation management
- Logistics operations optimization

**Technology Platform:** FleetFlow™ (Pre-launch logistics intelligence)

**Solicitation Keywords:**
- Logistics consulting
- Fleet optimization
- Transportation consulting
- Supply chain management
- Freight brokerage
- Dispatch services
- Route optimization
- Fleet management services

---

### **3. HEALTHCARE TRANSPORTATION SOLUTIONS**

#### **NEMT Program Development**
- Non-Emergency Medical Transportation
- Medicaid/Medicare enrollment support
- NEMT program implementation
- Healthcare logistics coordination
- Patient transport management
- Medical appointment transportation

#### **Operational Optimization**
- Healthcare transportation consulting
- Provider enrollment (PECOS/NPPES)
- CHAMPS system expertise
- CMS Connect integration

**Solicitation Keywords:**
- NEMT services
- Healthcare transportation
- Medical transportation
- Medicaid transport
- Medicare transport
- Patient transport services
- Ambulatory transport
- Healthcare logistics
- NEMT program management

---

### **4. BUSINESS CONTINUITY & CRISIS MANAGEMENT (DDI'S CORE IDENTITY)**

**Business continuity is not just a service line — it is DDI's foundational identity.** Every contract DDI manages is a business continuity engagement. This capability area provides dedicated continuity planning and crisis management for clients who need it as a standalone offering.

- Continuity of Operations (COOP) plan development
- Comprehensive disaster recovery planning
- Risk management and mitigation strategy
- Business continuity planning and testing
- Crisis management and emergency operations coordination
- Emergency preparedness consulting
- Incident response planning
- Operational resilience consulting
- Threat assessment and vulnerability analysis
- Recovery strategy development
- Technology-driven continuity solutions (leveraging DDI's proprietary AI platforms)
- Supply chain continuity and redundancy planning

**Powered By:** NEXUS platform — the same technology infrastructure that makes DDI's own operations unstoppable

**Solicitation Keywords:**
- Business continuity planning
- Disaster recovery services
- Crisis management
- Risk management
- Emergency preparedness
- Security consulting
- Incident response
- Resilience planning
- Continuity of operations (COOP)
- Operational resilience
- Business continuity consulting

---

### **5. CREDENTIALING & IDENTITY VERIFICATION SERVICES**

#### **Biometric Fingerprinting**
- Live scan electronic biometric fingerprinting (digital capture)
- Ink fingerprinting on FBI FD-258 cards
- SWFT electronic submission to DCSA/FBI (SWFT-authorized)
- ATF Electronic Fingerprint Transmission (EFT) file creation
- Mobile biometric fingerprinting (on-site at client facilities)
- Cross-state licensing fingerprinting
- Federal contractor security clearance fingerprinting

#### **Background Checks & Screening**
- Criminal history checks (local, state, federal)
- Employment and education verification
- Professional license verification
- Motor vehicle records and credit checks
- Sex offender registry checks
- Ongoing monitoring programs

#### **DOT Compliance**
- DOT regulations compliance
- Driver qualification files
- Safety compliance programs
- FMCSA compliance support

#### **Mobile Testing Programs**
- On-site testing services
- Mobile unit deployment
- Multi-location coordination

**Solicitation Keywords:**
- Federal compliance services
- Biometric fingerprinting
- Live scan fingerprinting
- Background check services
- Security clearance fingerprinting
- DOT compliance
- Credentialing services
- Identity verification
- SWFT fingerprinting

---

### **6. NOTARY & DOCUMENT AUTHENTICATION SERVICES**

#### **Notary Services**
- Michigan Commissioned Notary Public (active since April 2005 — 20+ years)
- CNTDA — Certified Notary & Trained Document Agent (estate planning, trusts, POA, healthcare directives)
- NPR — Notary Permit Runner (building permit expediting for contractors)
- Mobile notary services (nationwide coverage)
- Remote Online Notarization (RON) via ZigSig (nationwide coverage)
- Loan signing agent services (mortgage closings)
- Hospital, jail, and facility notarizations
- Government document notarization

#### **Apostille & Document Authentication**
- Apostille processing and coordination (Hague Convention authentication)
- Secretary of State document authentication
- Federal document authentication and certification
- International document legalization coordination
- Embassy/consulate document processing support

**Solicitation Keywords:**
- Notary services
- Mobile notary
- Document authentication
- Signing services
- RON services
- Notarial acts
- Apostille services
- Document legalization
- Biometric notarization

---

### **2. TRANSPORTATION SERVICES**

#### **Healthcare Transportation (NEMT)**
- Non-Emergency Medical Transportation
- Medicaid/Medicare transportation
- Ambulatory transport
- Wheelchair accessible transport
- Medical appointment coordination

#### **General Transportation**
- Freight brokerage (MC-1647572)
- Logistics coordination
- Fleet management
- Emergency transportation
- Government employee transport

**Solicitation Keywords:**
- NEMT services
- Medical transportation
- Non-emergency transport
- Patient transport
- Healthcare logistics
- Ambulatory services
- Wheelchair transport
- Freight brokerage
- Transportation logistics
- Fleet services

---

### **3. EMERGENCY SERVICES**

- Emergency logistics coordination
- Crisis response management
- Disaster recovery support
- Business continuity services
- Emergency transportation
- FEMA contract support
- State emergency management support

**Solicitation Keywords:**
- Emergency services
- Crisis management
- Disaster response
- Emergency logistics
- Business continuity
- FEMA support
- Emergency coordination
- Crisis transportation

---

### **4. OCCUPATIONAL HEALTH SERVICES**

#### **Drug & Alcohol Testing**
- DOT drug testing (Quest Diagnostics partnership)
- Pre-employment screening
- Random testing programs
- Post-accident testing
- Reasonable suspicion testing
- Return-to-duty testing
- Mobile testing services

#### **Urine Testing Programs**
- DOT-compliant collection
- Non-DOT testing
- On-site collection services
- Mobile testing units
- Chain of custody management

**Solicitation Keywords:**
- Drug testing services
- DOT testing
- Occupational health screening
- Pre-employment testing
- Urine analysis
- Mobile testing
- Substance abuse testing
- Drug screening programs
- Alcohol testing
- DOT compliance testing

---

### **5. BIOMETRIC FINGERPRINTING SERVICES**

- Live scan electronic biometric fingerprinting (digital capture)
- Ink fingerprinting on FBI FD-258 cards
- SWFT electronic submission to DCSA/FBI (SWFT-authorized)
- ATF Electronic Fingerprint Transmission (EFT) file creation
- FBI background checks and channeling
- State background checks and submissions
- Employment and licensing fingerprints
- Mobile biometric fingerprinting (on-site at client facilities)
- Government clearance and security fingerprinting
- Cross-state licensing fingerprinting

**Solicitation Keywords:**
- Biometric fingerprinting services
- Live scan fingerprinting
- Background check services
- FBI fingerprinting
- Employment screening
- Criminal background checks
- Security clearance fingerprinting
- Mobile fingerprinting
- SWFT fingerprinting
- Identity verification

---

### **6. DOCUMENT PREPARATION & SUPPORT**

- Business document preparation
- Government forms assistance
- Compliance documentation
- Contract document preparation
- Proposal preparation support
- Legal document formatting
- Records management support

**Solicitation Keywords:**
- Document preparation
- Administrative services
- Records management
- Document processing
- Forms processing
- Data entry services
- Document management
- Filing services

---

### **7. DNA TESTING & FORENSIC SERVICES**

- Court-admissible DNA testing (DDC partnership)
- Paternity testing
- Forensic DNA analysis
- Chain of custody services
- Legal DNA testing
- Immigration DNA testing

**Solicitation Keywords:**
- DNA testing services
- Paternity testing
- Forensic testing
- Court-admissible testing
- Legal DNA services
- Immigration testing

---

### **8. SURETY BONDS & FINANCIAL SERVICES**

- Federal contract bonds (SuretyCloud partnership)
- Bid bonds
- Performance bonds
- Payment bonds
- License and permit bonds
- Invoice factoring solutions (Bankers Factoring affiliate)

**Solicitation Keywords:**
- Surety bond services
- Contract bonds
- Bonding services
- Financial services
- Payment bonds
- Performance bonds

---

### **9. STAFFING SOLUTIONS**

- Temporary staffing
- Professional staffing
- Contract staffing
- Government agency staffing
- Administrative staff placement
- Specialized workforce solutions

**Solicitation Keywords:**
- Staffing services
- Temporary staffing
- Contract staffing
- Personnel services
- Workforce solutions
- Staff augmentation

---

### **10. VALUE-ADDED SUPPLIER / PRODUCT RESALE**

- Government supply contracts
- Product sourcing and procurement
- Value-added reseller services
- Equipment and supplies
- Municipal products
- Industrial supplies
- Office supplies and equipment
- Facilities supplies
- Safety equipment and supplies
- Fleet supplies and parts

**Solicitation Keywords:**
- Supply contracts
- Product supply
- Equipment supplier
- Materials supplier
- Value-added reseller
- Procurement services
- Product sourcing
- Government supplies

---

### **11. EMERGENCY HOUSING SERVICES**

- Emergency shelter coordination
- Temporary housing solutions
- Disaster housing programs
- Emergency accommodation services
- Displaced persons housing
- Crisis housing management
- Transitional housing coordination
- Emergency hotel/motel procurement
- Temporary shelter setup

**Solicitation Keywords:**
- Emergency housing
- Temporary housing
- Disaster housing
- Emergency shelter
- Crisis housing
- Transitional housing
- Emergency accommodation
- Shelter services

---

### **12. WAREHOUSING & STORAGE SERVICES**

- Warehouse management
- Storage solutions
- Inventory management
- Distribution services
- Cold storage
- Secure storage facilities
- Government property storage
- Emergency supply storage
- Equipment storage
- Records storage

**Solicitation Keywords:**
- Warehousing services
- Storage services
- Warehouse management
- Distribution center
- Inventory storage
- Secure storage
- Cold storage
- Records storage
- Property storage

---

### **13. DISASTER RELIEF SERVICES**

- Emergency response coordination
- Disaster recovery support
- Relief supply distribution
- Emergency logistics coordination
- Crisis response services
- Mass care support
- Emergency feeding programs
- Disaster supply procurement
- Recovery operations support
- FEMA coordination services

**Solicitation Keywords:**
- Disaster relief
- Emergency relief
- Disaster response
- Crisis response
- Emergency services
- Disaster recovery
- Relief operations
- FEMA support
- Mass care
- Emergency coordination

---

### **14. PROFESSIONAL SERVICES (Comprehensive)**

- Administrative support services
- Consulting services
- Strategic planning
- Operational optimization
- Process improvement
- Performance management
- Quality assurance
- Compliance consulting
- Training and development
- Technical assistance

**Solicitation Keywords:**
- Professional services
- Administrative support
- Consulting services
- Management consulting
- Operational support
- Strategic consulting
- Business process services
- Technical assistance

---

## 🎯 PRIMARY SERVICE CATEGORIES FOR EDWOSB SEARCHES

When mining EDWOSB opportunities, focus on solicitations for:

### **NAICS Codes to Monitor:**

**Professional Services:**
- **541611** - Administrative Management and General Management Consulting Services
- **541618** - Other Management Consulting Services
- **541990** - All Other Professional, Scientific, and Technical Services
- **541690** - Other Scientific and Technical Consulting Services

**Administrative & Support Services:**
- **561110** - Office Administrative Services
- **561210** - Facilities Support Services
- **561320** - Temporary Help Services
- **561421** - Telephone Answering Services
- **561439** - Other Business Service Centers
- **561499** - All Other Business Support Services
- **561990** - All Other Support Services (notary, document prep)

**Investigation & Security:**
- **561611** - Investigation Services (background checks, fingerprinting)
- **561621** - Security Systems Services
- **561622** - Locksmiths

**Healthcare & Testing:**
- **621511** - Medical Laboratories
- **621999** - All Other Miscellaneous Ambulatory Health Care Services (drug testing)
- **624230** - Emergency and Other Relief Services

**Transportation:**
- **485991** - Special Needs Transportation
- **485999** - All Other Transit and Ground Passenger Transportation (NEMT)
- **484110** - General Freight Trucking, Local
- **484121** - General Freight Trucking, Long-Distance, Truckload
- **488510** - Freight Transportation Arrangement

**Emergency Services:**
- **561990** - All Other Support Services (emergency management)
- **624230** - Emergency and Other Relief Services
- **721199** - All Other Traveler Accommodation (emergency housing)
- **721214** - Recreational and Vacation Camps (shelter services)

**Compliance & Credentialing:**
- **541990** - All Other Professional, Scientific, and Technical Services (compliance)
- **561990** - All Other Support Services (credentialing)

**Warehousing & Storage:**
- **493110** - General Warehousing and Storage
- **493120** - Refrigerated Warehousing and Storage
- **493130** - Farm Product Warehousing and Storage
- **493190** - Other Warehousing and Storage

**Supply & Distribution:**
- **423450** - Medical, Dental, and Hospital Equipment and Supplies Merchant Wholesalers
- **423990** - Other Miscellaneous Durable Goods Merchant Wholesalers
- **424990** - Other Miscellaneous Nondurable Goods Merchant Wholesalers
- **425120** - Wholesale Trade Agents and Brokers

---

## 📋 SOLICITATION SEARCH STRATEGY

### **SAM.gov Search Terms (EDWOSB Set-Asides):**

**Project Management & Consulting:**
- "project management"
- "program management"
- "contract management"
- "PMO services"
- "management consulting"
- "strategic planning"
- "operational consulting"
- "performance management"

**Logistics & Fleet:**
- "logistics services"
- "freight brokerage"
- "fleet management"
- "transportation management"
- "supply chain"
- "route optimization"
- "dispatch services"

**Healthcare Services:**
- "NEMT"
- "medical transportation"
- "non-emergency transport"
- "patient transport"
- "healthcare transportation"
- "ambulatory transport"
- "Medicaid transport"
- "Medicare transport"

**Health/Testing Services:**
- "drug testing"
- "occupational health"
- "pre-employment screening"
- "DOT testing"
- "mobile testing"
- "substance abuse testing"
- "DNA testing"
- "paternity testing"
- "forensic testing"

**Background/Security Services:**
- "fingerprinting"
- "background check"
- "live scan"
- "security screening"
- "employment screening"
- "credentialing"

**Emergency & Continuity:**
- "emergency services"
- "disaster response"
- "crisis management"
- "business continuity"
- "emergency logistics"
- "disaster recovery"
- "COOP"
- "emergency preparedness"

**Compliance Services:**
- "federal compliance"
- "DOT compliance"
- "regulatory compliance"
- "FMCSA compliance"
- "safety compliance"

**Administrative Services:**
- "notary services"
- "document preparation"
- "administrative support"
- "records management"
- "signing services"
- "office support"

**Staffing Services:**
- "staffing services"
- "temporary staffing"
- "contract staffing"
- "personnel services"
- "workforce solutions"

**Financial Services:**
- "surety bonds"
- "contract bonds"
- "bonding services"

**Product Supply & Procurement:**
- "supply contracts"
- "product supply"
- "equipment supplier"
- "materials supplier"
- "value-added reseller"
- "government supplies"
- "procurement services"
- "industrial supplies"
- "municipal supplies"
- "office supplies"

**Emergency Housing:**
- "emergency housing"
- "temporary housing"
- "disaster housing"
- "emergency shelter"
- "crisis housing"
- "temporary shelter"
- "emergency accommodation"
- "transitional housing"
- "displaced persons housing"

**Warehousing & Storage:**
- "warehousing services"
- "storage services"
- "warehouse management"
- "distribution services"
- "inventory storage"
- "cold storage"
- "secure storage"
- "records storage"
- "government storage"

**Disaster Relief:**
- "disaster relief"
- "emergency relief"
- "disaster response"
- "relief operations"
- "emergency supplies"
- "disaster recovery"
- "mass care"
- "emergency feeding"
- "relief coordination"
- "FEMA services"

---

## ❌ WHAT DEE DAVIS INC DOES NOT DO

**Avoid solicitations for:**
- Grant writing services (not expertise)
- IT/Software development (not service line)
- Construction/trades (not licensed)
- Engineering services (not expertise)
- Architectural services (not expertise)
- Legal services (not attorneys)
- Accounting/auditing (not CPAs)

---

## ✅ IDEAL EDWOSB OPPORTUNITIES

**Look for solicitations that need:**

1. **Project Management Services**
   - Government program management
   - Contract management services
   - PMO support
   - Strategic planning services
   - Performance management

2. **Mobile/On-site Services**
   - Drug testing at government facilities
   - Fingerprinting at multiple locations
   - Mobile notary for government offices
   - On-site compliance services

3. **Transportation Contracts**
   - VA medical transport
   - Medicare/Medicaid NEMT
   - Government employee shuttle services
   - Emergency transport services
   - Freight brokerage services
   - Fleet management contracts

4. **Occupational Health Programs**
   - DOT testing for government fleets
   - Pre-employment screening for agencies
   - Ongoing drug testing programs
   - DNA testing services
   - Forensic testing

5. **Administrative Support Services**
   - Document preparation for agencies
   - Notary services for government offices
   - Records management support
   - Office administrative support
   - Staffing services

6. **Emergency & Continuity Services**
   - FEMA support contracts
   - State emergency management support
   - Crisis transportation coordination
   - Business continuity planning
   - Disaster recovery services

7. **Compliance Services**
   - Federal compliance support
   - DOT compliance programs
   - Credentialing services
   - Background check programs

8. **Logistics & Fleet Services**
   - Transportation management
   - Supply chain consulting
   - Fleet optimization
   - Route planning services

9. **Consulting Services**
   - Management consulting
   - Operational optimization
   - Strategic planning
   - Process improvement
   - Performance consulting

10. **Supply & Procurement Contracts**
   - Government supply contracts
   - Product procurement
   - Equipment supply
   - Municipal supplies
   - Industrial supplies
   - Office equipment/supplies
   - Safety equipment
   - Fleet supplies

11. **Emergency Housing Contracts**
   - Disaster housing programs
   - Emergency shelter services
   - Temporary housing
   - Crisis accommodation
   - Transitional housing
   - FEMA housing support

12. **Warehousing & Storage Contracts**
   - Government warehouse services
   - Emergency supply storage
   - Records storage
   - Equipment storage
   - Inventory management
   - Distribution center operations

13. **Disaster Relief Contracts**
   - FEMA disaster relief
   - Emergency response
   - Relief supply distribution
   - Mass care services
   - Recovery operations
   - Emergency feeding programs

---

## 💻 PROPRIETARY TECHNOLOGY PLATFORMS

### **NEXUS - Integrated Business Automation Platform**

**Overview:** NEXUS is DEE DAVIS INC's proprietary AI-powered business automation platform integrating three specialized systems for complete business operations management.

**Three Core Systems:**

1. **GPSS (Government Procurement & Sales System)**
   - Government opportunity tracking and mining (SAM.gov, state portals)
   - AI-powered proposal generation
   - Intelligent pricing algorithms
   - Compliance checking
   - Contact and product catalog management
   - Win/loss analytics and pipeline management

2. **ATLAS PM (Project Management)**
   - Detailed below

3. **DDCSS (Corporate Sales System)**
   - Client avatar building and qualification
   - Success path mapping
   - PitchMap script generation
   - AI email response analysis
   - Corporate pipeline management
   - Multi-sector sales systems

**Additional Modules:**
- Invoice generation and management
- Document generation (RFPs, RFQs, proposals)
- Calendar automation
- Transportation logistics coordination

---

### **ATLAS PM - AI-Powered Project Management System (within NEXUS)**

**Core Capabilities:**
- **Project Management:** Full CRUD for projects with real-time tracking and analytics
- **AI RFP Analysis:** Automated analysis of RFPs with win probability assessment, risk identification, and strategy recommendations
- **WBS Generator:** AI-powered Work Breakdown Structure generation from project requirements
- **Change Order Management:** Track and analyze change requests with AI impact assessment
- **Task Board:** Kanban-style task management with multiple views (board, timeline, calendar, list)
- **Budget Tracking:** Real-time budget monitoring and forecasting
- **Document Management:** RFP document processing and analysis
- **Project Logs:** Comprehensive activity tracking and audit trails
- **Analytics Dashboard:** Real-time project metrics, completion rates, and performance analytics

**AI Integration:**
- Claude AI for RFP analysis (requirements extraction, risk assessment, win strategies)
- Automated WBS generation based on project scope
- Change order impact analysis (budget, timeline, resources)
- Intelligent task suggestions and auto-generation

**Data Management:**
- Airtable-backed persistence
- Real-time synchronization across all systems
- REST API for integration with external systems
- DDCSS integration for corporate RFP conversion

**Competitive Advantage:** Delivers AI-powered project intelligence, automated work breakdown structures, and predictive analytics that conventional project management tools cannot provide

### **FleetFlow™ - Logistics Intelligence (Pre-Launch)**
- Advanced AI & predictive analytics
- Real-time operational intelligence
- Route optimization
- Fleet management
- Carrier coordination
- Supply chain optimization

**Competitive Advantage:** Next-generation logistics technology for superior operational outcomes

---

## 🎯 COMPETITIVE ADVANTAGES

**Highlight in proposals:**

- ✅ **Business continuity-focused contract management firm** — every engagement ensures operations never stop
- ✅ **8+ proprietary AI technology platforms (founder-built)** — NEXUS, GPSS, ATLAS PM, FleetFlow™, COMPASS™, PRISM, DDCSS, VERTEX, GBIS — 5-10 year competitive advantage, SBIR/STTR eligible, future SaaS licensing potential
- ✅ **90% automation of contract lifecycle** — Fortune 500-level operations at small business scale
- ✅ **EDWOSB/WOSB/WBE/MBE/SBE/WBENC certified** — diversity advantage with access to $30B+ reserved market
- ✅ **SupplierGateway Certificate of Diverse Ownership** — DBE, MBE, SB, SDB, Woman Owned (valid through July 2028) — visible to Fortune 500 and large prime supplier diversity searches
- ⚠️ **Coupa Verified Supplier** (Cert ID: 7002160) — EXPIRED 11/2025 — needs renewal for corporate procurement visibility
- ✅ **E-Verify certified** — employment eligibility verification, required by many federal contracts
- ✅ **SWFT authorized** — Secure Web Fingerprint Transmission to DCSA/FBI
- ✅ **Michigan Personnel Agency License #6016004743** — state-licensed staffing and personnel services
- ✅ **Michigan Property Pre-License** — real estate and property services qualified
- ✅ **Property Data Collection certified** — residential property inspections and data collection (Walitt Solutions, 2022)
- ✅ **Project Management for Government Projects** — certified, on file
- ✅ **TWIC credentialed** — Transportation Worker Identification Credential (TSA secure facility access)
- ✅ **Multiple service lines** — business continuity across every sector
- ✅ **Mobile/on-site capabilities** — services delivered at client location
- ✅ **Federal credentials** — CAGE: 8UMX3, UEI: HJB4KNYJVGZ1, NPI: 1538939111
- ✅ **Transportation credentials** — MC-1647572, DOT: 4250594
- ✅ **Established partnerships** — Quest Diagnostics, DDC, Uber Health, Freight 1st Direct, SuretyCloud, ZigSig
- ✅ **Multi-state capability** — National service delivery
- ✅ **CMMC-AB certified** — Cybersecurity compliance, DoD-ready

---

## 📞 PARTNERSHIP STRATEGY

**For services outside core competency:**
- Prime contractor role (project management, coordination)
- Subcontract specialized work (e.g., grant writing)
- Leverage ATLAS PM for coordination
- Use EDWOSB status as prime

---

**USE THIS REFERENCE when mining EDWOSB opportunities to ensure proper fit analysis!**

**Remember: DDI is a contract management firm and business operating system focused on ALL forms of business continuity — powered by proprietary AI technology. Every opportunity should be evaluated through that lens.**
